...
### [x] P6 freshness / repriorización — hecho
...
- Inicio 2026-06-09: se agregara una verificacion local/CI que lea `data/*.json`, consulte Mercado Libre, escriba metadata de freshness y falle si una review queda sin estado verificable.
- Resultado 2026-06-09: `scripts/n8n-ephemeral/prepare-workflows.mjs` genera workflow `AfiliadosML - Freshness` (`freshnessAfML2026`). El workflow refresca token ML, consulta `/products/{catalog_id}/items`, escribe `freshness` en cada `data/*.json`, marca stale por `inactive_listing`, `no_stock`, `missing_price`, `price_increased_20pct`, `api_error` o `no_seller_items`, y sube prioridad de candidatos pendientes cuyo `source_slug` quedo stale.
- Integracion: `RUN_FRESHNESS=true` ejecuta Freshness en `scripts/n8n-ephemeral/run-cycle.sh`; GitHub Actions lo activa en el ciclo diario de 7am.
- Verificacion live 2026-06-09: se ejecuto `n8n execute --id=freshnessAfML2026`; los 5 reviews quedaron con `freshness.status=active_inferred`, `is_available=true`, `stale=false`, `price_delta_pct=0`, `stale_count=0`.
- Verificacion local: `rtk npm run n8n:prepare`, `rtk npm run review:audit` (5 archivos, 0 warnings), `rtk npm run build`.
- Nota operativa: no habia producto caido real en `data/` al momento de la prueba; las ramas stale quedan listas para el primer item inactivo/sin stock/caro que detecte ML.
